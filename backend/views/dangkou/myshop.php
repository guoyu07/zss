...
<!---->
        ...
        <td><a href="<?= yii::$app->urlManager->createUrl(['dangkou/mydangkou'])?>&shop_id=<?= Html::encode($value["shop_id"])?>" class="makeit" value="<?= Html::encode($value["shop_id"])?>">门店档口</a></td>                
                </tr>
                <?php  }?>

               <?php }?>
                <?php if (empty($allinfo)) { ?>
                  <font color="red">暂时没有门店</font>
                <?php } ?>
              </tbody>
            </table>
          </div>
