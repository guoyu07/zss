...
<!---->
<div id="smallt" value="0"><h5 id="small"><a href="<?= Yii::$app->urlManager->createUrl(['dangkou/index']); ?>"><button class="btn btn-info" id="ac">未做订单</button></a><a href="<?= Yii::$app->urlManager->createUrl(['dangkou/overmenu']); ?>"><button class="btn btn-primary" id="ac">已做订单</button></a></h5></div>
    <div id="searchdiv">
    <!--time start -->
     <!--   <div id="time">
...
<!---->
 ...
 <?php if($value["is_make"]==1){ ?>
    <a href="javascript:void(0)" class="yes" value="<?= Html::encode($value["info_id"])?>"><button class="btn btn-primary" id="ac">完成</button></a>
  <?php } ?> 

  <?php if($value["is_make"]==0){ ?>
   <span id="x<?= Html::encode($value["info_id"])?>"> <a href="javascript:void(0)" class="loding" value="<?= Html::encode($value["info_id"])?>">制作中</a></span>
  <?php } ?> 
        ...
        $(document).on("click",".loding",function(){
          ...
          $.ajax({
       type: "GET",
       url: "<?= Yii::$app->urlManager->createUrl(['dangkou/fixmake']); ?>",
       data: "id="+id,
