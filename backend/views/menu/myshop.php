...
<!---->
<div id="small" value="0"><h5 id="smallfont">我的店面</h5></div>
    <div id="searchdiv">
    <!--time start -->
     <!--   <div id="time">
            ...
            <table class="table table-bordered data-table" style="overflow:scroll; font-size:15px;font-family:楷体">
              ...
              <?php foreach ($allinfo as $key => $value) {?>
                ...
                <tr class="gradeX">
                  ...
                  <td>
                    <?= Html::encode($value["shop_id"])?>
                  </td>
        <td>                    <?= Html::encode($value["shop_name"])?>
</td>
    <td><a href="<?= yii::$app->urlManager->createUrl(['menu/listshop'])?>&id=<?= Html::encode($value["shop_id"])?>">管理该店菜品</a></td>                
                </tr>
                <?php  }?>
               <?php }?>
                <?php if (empty($allinfo)) { ?>
                  <font color="red">该店主没有店面</font>
                <?php } ?>
              </tbody>
            </table>
          </div>
