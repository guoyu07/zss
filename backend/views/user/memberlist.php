...
#show{display: none;  position: absolute;  top: 15%;  left: 10%;  width: 60%;  height: 80%;  padding: 8px;  border: 8px solid #E8E9F7;  background-color: white;  z-index:1002;  overflow: auto;}   
</style>
...
<div class="widget-box">
    ...
    <div id="searchdiv">
         <!--start-top-serch-->  
         <div id='bt1'>
            <a   href='<?php echo Yii::$app->urlManager->createUrl(['user/admember']);?>'><button class='btn btn-primary' style="">添加</button></a>&nbsp;&nbsp;<button class='btn1 btn btn-info'>删除</button>&nbsp;&nbsp;<button class='che btn btn-info'>全选</button>
         </div>
        <div id="search">   
         
                <input type="text" placeholder="搜索人名..." id="word"/>
              ...
              <tbody>
                  <?php foreach($list as $models){;?>
                    <tr class="gradeX">
                      ...
                      <td><?php if(isset($models['user_experience'])){ echo $models['user_experience'];}else{ echo 0;}?></td> 
                      <td><?php if(isset($models['company_name'])){ echo $models['company_name'];}?></td> 
                      <td><?php if(isset($models['vip_name'])){ echo $models['vip_name'];}?></td>
                      <td class="active"  value='<?= Html::encode($models['user_id'])?>'  type="<?= Html::encode($models['user_status'])?>">          
                     
                     <?php if($models['user_status'] == 1){?>
...
<script>
    ...
    $(function(){
        ...
        $(document).on("click",".tip-bottom",function(){
            var search = $("#word").val();
            $.get("<?php echo Url::to(['user/findmember']);?>",{search : search},function(data){
                             if( data['code'] == '200' ){
		...
		//询问框
            layer.confirm('您确定要删除吗？', {
              btn: ['删除','取消'] //按钮
                ...
                $.get("<?php echo Yii::$app->urlManager->createUrl(['user/dellist']);?>",{cid : cid},function(data){
                    if( data['code'] == '200' ){
        ...
        $(document).on("click",".btn1",function(){
            ...
            layer.confirm('您确定要删除吗？', {
              btn: ['删除','取消'] //按钮
                ...
                $.get("<?php echo Url::to(['user/dellist']);?>",{cid : mid},function(data){
                    if( data['code'] == '200' ){
                       layer.msg('删除成功', {icon: 1});
                       location.reload()
                    }else if(data['code'] == '400'){
        ...
        $(document).on("click",".upd",function(){
           ...
           // var zhe = "";
            $.ajax({
               type: "GET",
               url: "<?php echo Url::to(['user/moreuser']);?>",
               data: "cid="+cid,
                beforeSend: function(){
                        //$("#loading").show();
                    },
        ...
        $(document).on("click",".active",function(){
            ...
            $.get("<?php echo Url::to(['user/userstatus'])?>",{cid : $(this).attr("value"),status : status},function(data){
                if( data['code'] == '200' ){
                     //修改成功
                    }else if(data['code'] == '400'){
                        alert('修改状态失败')
