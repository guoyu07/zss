...
<?php if($flash = Yii::$app->session->getFlash('success')) {?>
               <input type="hidden" id="errorfix" value="<?= $flash?>">
<?php  } ?>
<?= Html::jsFile('./assets/layer/laydate/laydate.js')?>
<style>
label{display:none}
#searchdiv{
    display:none;
...
.btn-group{margin-right:15px; float:right;}
 ...
 #show{display: none;  position: absolute;  top: 15%;  left: 10%;  width: 65%;  height: 65%;   border: 8px solid #E8E9F7;  background-color: white;  z-index:1002;  overflow: auto;margin:0 auth;}
</style>
<div class="widget-box">
    ...
    <div id="searchdiv">
         <!--start-top-serch-->  
         <div id='bt1'>
            <a   href='<?php echo Yii::$app->urlManager->createUrl(['user/adcompany']);?>'><button class='btn btn-primary'>添加</button></a>&nbsp;&nbsp;<button class='btn1 btn btn-info'>删除</button>&nbsp;&nbsp;<button class='che btn btn-info'>全选</button>
         </div>
        <div id="search">   
         
                <input type="text" placeholder="搜索公司名..." id="word"/>
              ...
              <tbody>
                  <?php foreach($partner as $models){  ?>
                    <tr class="gradeX">
                      ...
                      <td><?= Html::encode($models['company_discount']);?></td>    
                      <td class="center"><?= Html::encode($models['company_price']);?></td>          
                      <td><?= Html::encode($models['company_subtract']);?></td>                  
                      <td>
                      ...
                      <td class="active"  value='<?= Html::encode($models['company_id'])?>' type="<?= Html::encode($models['company_status'])?>">
                     <?php if($models['company_status'] == 1){?>
                     <img src="./assets/ico/png/34.png" width=20 height=20 /> 
                         <?php }else{?>
                      <img src="./assets/ico/png/40.png" width=20 height=20/>
                         <?php }?>      
                     </td>
                     <td><a href="javascript:void(0);" class="upd"  id="d<?= Html::encode($models['company_id']);?>" value="<?= Html::encode($models['company_id']);?>"><img src="<?php echo Yii::$app->request->baseUrl;?>/assets/order/search.png"  width=20 height=20/></a>&nbsp;&nbsp;<a   href="<?php echo Yii::$app->urlManager->createUrl(['user/updcompany'])?>&cid=<?= Html::encode($models['company_id']);?>"><img src="<?php echo Yii::$app->request->baseUrl;?>/assets/actions/edit.png"/></a>&nbsp;&nbsp;&nbsp;<a  href="javascript:void(0)" class="del" id="d<?= Html::encode($models['company_id']);?>" value="<?= Html::encode($models['company_id']);?>"><img src="<?php echo Yii::$app->request->baseUrl;?>/assets/actions/delete.png"/></a></td>
                    </tr>
                  <?php };?>
              </tbody>
            </table>
...
<script>
    $(function(){
        ...
        $(document).on("click",".tip-bottom",function(){
            var search = $("#word").val();
            $.get("<?php echo Url::to(['user/findcompany']);?>",{search : search},function(data){
                if( data['code'] == '200' ){
		...
		//询问框
            layer.confirm('您确定要删除吗？', {
              btn: ['删除','取消'] //按钮
                ...
                $.get("<?php echo Url::to(['user/delcompany']);?>",{cid : cid},function(data){
                    if( data['code'] == '200' ){
        ...
        $(document).on("click",".btn1",function(){
            ...
            layer.confirm('您确定要删除吗？', {
              btn: ['删除','取消'] //按钮
                ...
                $.get("<?php echo Url::to(['user/delcompany']);?>",{cid : mid},function(data){
                    if( data['code'] == '200' ){
                       layer.msg('删除成功', {icon: 1});
                       location.reload()
                    }else if( data['code'] == '400' ){
        ...
        $(document).on("click",".upd",function(){
            ...
            $.ajax({
               type: "GET",
               url: "<?php echo Url::to(['user/morecompany']) ?>",
               data: "cid="+cid,
                beforeSend: function(){
                       // $("#loading").show();
                    },
       ...
       //关闭遮罩层
        ...
        $(document).on("click",".active",function(){
            ...
            $.get("<?php echo Url::to(['user/comstatus'])?>",{cid : $(this).attr("value"),status : status},function(data){
                    if( data['code'] == '200' ){
                     //修改成功
                    }else if( data['code'] == '400' ){
                        alert('修改状态失败')
