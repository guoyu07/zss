        ...
        <div class="widget-content nopadding" >
		  ...
		  <div class="control-group">
              <label class="control-label">
					...
					<span>&nbsp;&nbsp;&nbsp;店长姓名&nbsp;:&nbsp;<?= Html::encode($username); ?></span>
					</button>
			 </label>
...
=======
	<div class="control-group">
	  <label class="control-label"><button class="btn btn-primary" disabled >配送员</button></label>
	  <div class="controls">
	  	<?php if ($all_deli) { ?>
	  		<?php foreach($all_deli as $key=>$value): ?>
				<?php if(in_array($value['id'], $in_deli)): ?>
					<input type="checkbox" disabled checked name="ShopinfoForm[deli][]" value="<?= $value['id']; ?>"><?= $value['username']; ?><br>
				<?php else: ?>
					<input type="checkbox" disabled name="ShopinfoForm[deli][]" value="<?= $value['id']; ?>"><?= $value['username']; ?><br>
				<?php endif; ?>
			<?php endforeach; ?>
	  	<?php } ?>
 	...
 	<div class="control-group">
		  ...
		  <div class="controls"  style=" padding-top:20px;">
			...
			<div class="control-group" >
              ...
              <div class="controls">
               <button type="button" class="btn btn-success">修改我的店铺信息</button>
              </div>
            </div>
</div>
<!-- footer end -->
...
$(document).on("click","button[type=button]",function(){
	shop_id = $("#shop_id").text();
	location.href ="<?= Url::to(['shop/update-shop']);?>&id="+shop_id;
});
</script>
