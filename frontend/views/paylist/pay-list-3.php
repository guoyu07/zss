...
<div class="content">
<form method="post" action="<?= yii::$app->urlManager->createUrl(['paylist/paylist3'])?>">
<input type="hidden" id="tt" name="_csrf" value="<?php echo \Yii::$app->request->getCsrfToken() ?>">
		...
		<div style="text-align:center;display:none" id="callto"><font color="red">余额不足</font></div>
        	<div class="payment-info-takeout">
            	<label>添加预定取餐时间</label>
				<select name='add_hour' style="border:none;">
					<?php for($i=1;$i<=24;$i++){ ?>
						<option value="<?php echo $i;?>"><?php echo $i;?></option>
					<?php }?>
				</select>时
				<select name='add_min'  style="border:none;">
					<?php for($i=1;$i<6;$i++){ ?>
    ...
    <footer style="background:#fff;">
    	...
    	<button style="float:right; font-size:18px;width:89px; height:49px; background:#81181d; color:#fff; line-height:49px;" id="list3">提交订单</button><!--提交自提-->
		
        <span style="float:right;line-height:49px; font-size:18px; color:#81181d; margin-right:20px;  ">¥
				<?php 
					echo $realPrice+$lunchbox;;
				?>
		</span><input type="hidden" value="<?php echo $realPrice?>" id="all"/>
